...
class HuggingFaceBlogDownloader:
    ...
    def extract_authors(self, soup):
        ...
        try:
            ...
            for author in json_matches:
                ...
                if (clean_author and 
                    clean_author not in authors and 
                    len(clean_author) > 1 and
                    clean_author.lower() not in ['update on github', 'upvote', 'guest', 'avatar', 'icon', 'logo', 'favicon', 'open/acc', 'intel', 'huggingface', 'microsoft', 'google', 'amazon', 'meta', 'openai'] and
                    not any(username in clean_author.lower() for username in ['juliensimon', 'echarlaix', 'ofirzaf', 'imargulis', 'guybd', 'moshew', 'tonic', 'lunarflu', 'jiqing', 'katarinayuan', 'sywangyi', 'matrixyao', 'chrisallenming', 'kding1'])):
    ...
    def extract_main_content(self, soup):
        ...
        if not main_content:
            ...
            content_divs = soup.find_all('div', class_=re.compile(r'content|post|article|main|prose', re.I))
            if content_divs:
        ...
        for element in soup.find_all(string=True):
            if element.parent:
                ...
                if any(keyword in text for keyword in [
                    'update on github', 'upvote', 'guest', 'avatar', 'juliensimon', 
                    'echarlaix', 'ofirzaf', 'imargulis', 'guybd', 'moshew'
                ]):
                    # Remove the parent element if it's a small container
                    parent = element.parent
                    if parent.name in ['span', 'a', 'button', 'div'] and len(parent.get_text().strip()) < 100:
                        parent.decompose()
        ...
        while current and current != first_image:
            if hasattr(current, 'name') and current.name:
                ...
                if (current.name in ['div', 'nav', 'header'] and 
                    ('back' in current.get_text().lower() or 
                     'navigation' in current.get_text().lower() or
                     'breadcrumb' in current.get_text().lower())):
    ...
    def remove_complex_structures(self, soup):
        ...
        for element in empty_elements:
            if not element.get_text().strip() and not element.find_all(['img', 'a', 'p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6']):
                element.decompose()
    ...
    def download_post(self, post):
        ...
        try:
            ...
            if post.get('authors'):
                ...
                authors_tag['style'] = "color: #666; font-style: italic; margin-bottom: 1em;"
...
def main():
    ...
    parser.add_argument('--urls', type=str, default='huggingface-blog-urls.txt',
