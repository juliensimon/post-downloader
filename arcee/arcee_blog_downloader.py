#!/usr/bin/env python3
"""
Arcee Blog Post Downloader CLI Tool

Downloads Arcee blog posts from arcee-blog-urls.txt, extracts main content,
...
class ArceeBlogDownloader:
    ...
    def extract_date(self, soup, url):
        ...
        # First try to extract from Arcee's blog header format
        # Look for the date pattern in the blog header - the date is split across multiple divs
        html_content = str(soup)

        # Look for the pattern: day number, then month name, then year
        day_pattern = re.compile(
            r'>(\d{1,2})</div>.*?>(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)</div>.*?>(\d{4})',
            re.IGNORECASE | re.DOTALL,
        )
        date_match = day_pattern.search(html_content)
        if date_match:
            ...
            return f"{year}-{month_num}-{day:0>2}"

        # Alternative pattern for when date elements are more separated
        day_alt_pattern = re.compile(
            r'>(\d{1,2})</div>.*?font_modern-gothic-mono">(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)</div>.*?font_modern-gothic-mono">(\d{4})',
            re.IGNORECASE | re.DOTALL,
        )
        date_match = day_alt_pattern.search(html_content)
        if date_match:
            day, month, year = date_match.groups()
            month_map = {
                'jan': '01',
    ...
    def extract_main_content(self, soup):
        ...
        # Try different selectors for Arcee blog main content
        selectors = [
            'section.blog_body .richtext',
            '.blog_body .richtext',
            'section.blog_body',
            '.blog_body',
        ...
        if not main_content:
            ...
            content_divs = soup.find_all(
                'div', class_=re.compile(r'content|post|article|main|richtext', re.I)
            )
            if content_divs:
    ...
    def remove_arcee_content(self, soup):
        """Remove Arcee-specific content and branding"""
        # Remove the entire Arcee CTA sections
        cta_sections = soup.find_all('div', class_=re.compile(r'blog-cta|cta', re.I))
        for cta in cta_sections:
            cta.decompose()

        # Remove tip sections
        tip_sections = soup.find_all('div', class_=re.compile(r'blog-tip|tip', re.I))
        for tip in tip_sections:
            tip.decompose()

        # Remove break lines
        break_lines = soup.find_all(
            'div', class_=re.compile(r'break-line|breakline', re.I)
        )
    ...
    def process_images(self, soup, base_url, post_folder):
        ...
        for img in images:
            ...
            # Skip Arcee branding images
            if 'arcee' in src.lower() or 'logo' in src.lower():
                img.decompose()
                continue
    ...
    def download_post(self, post):
        ...
        try:
            ...
            # Remove Arcee-specific content
            self.remove_arcee_content(new_soup)
...
def main():
    parser = argparse.ArgumentParser(description='Download Arcee blog posts')
    parser.add_argument(
        '--urls',
        type=str,
        default='arcee-blog-urls.txt',
    ...
    downloader = ArceeBlogDownloader(args.output)
